...
public class Establish extends BaseActivity{
		...
		if (!TextUtils.isEmpty(mid)) {
			...
			if (mei.getSound().equals("1")) {
				...
				for (final NoteFile nf : list) {
					LinearLayout ll_detail_record_item = (LinearLayout) View.inflate(Establish.this, R.layout.ll_detail_record_item, null);
					int time = Integer.parseInt(nf.getTime());
					RelativeLayout rl_view_player = (RelativeLayout) ll_detail_record_item.findViewById(R.id.rl_view_player);
					LinearLayout.LayoutParams lp = null;
					if (0 < time && time <= 15) {
						lp = new LinearLayout.LayoutParams(UIUtil.getInstance(Establish.this).convertDIP2PX(190), LinearLayout.LayoutParams.WRAP_CONTENT);
//						rl_view_player.getLayoutParams().width = ;
					}
					else if (15 < time && time <= 30) {
						lp = new LinearLayout.LayoutParams(UIUtil.getInstance(Establish.this).convertDIP2PX(220), LinearLayout.LayoutParams.WRAP_CONTENT);
//						rl_view_player.getLayoutParams().width = UIUtil.getInstance(Establish.this).convertPX2DIP(220);
					}
					else if (30 < time && time <= 45) {
						lp = new LinearLayout.LayoutParams(UIUtil.getInstance(Establish.this).convertDIP2PX(250), LinearLayout.LayoutParams.WRAP_CONTENT);
//						rl_view_player.getLayoutParams().width = UIUtil.getInstance(Establish.this).convertPX2DIP(250);
					}
					else if (45 < time && time <= 60) {
						lp = new LinearLayout.LayoutParams(UIUtil.getInstance(Establish.this).convertDIP2PX(280), LinearLayout.LayoutParams.WRAP_CONTENT);
//						rl_view_player.getLayoutParams().width = UIUtil.getInstance(Establish.this).convertPX2DIP(280);
					}
					rl_view_player.setLayoutParams(lp);
					...
					TextView tv_detail_time = (TextView) ll_detail_record_item.findViewById(R.id.iv_view_player_time);
					rl_view_player.setTag(nf.getMuris());
			        int f = time/60;
			        int m = time%60;
			        tv_detail_time.setTag(time);
	...
	private void initRecorder() {
		mp3Recorder = new Mp3Recorder(Establish.this, new RecordListener() {
...
//					rl_view_player.getLayoutParams().width = UIUtil.getInstance(Establish.this).convertPX2DIP(280);
				...
				TextView tv_detail_time = (TextView) ll_detail_record_item.findViewById(R.id.iv_view_player_time);
				rl_view_player.setTag(file.getAbsolutePath());
				tv_detail_time.setTag(time);
				tv_detail_time.setText(f+":"+m+"'");
...
//						}
	...
	public void playRecord(View view) {
		try {
			...
			PlayStone.getInstance().play(path, (RelativeLayout) view);
