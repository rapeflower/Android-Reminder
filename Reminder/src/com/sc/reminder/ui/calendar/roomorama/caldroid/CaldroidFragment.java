...
import com.sc.reminder.ui.calendar.antonyt.infiniteviewpager.InfinitePagerAdapter;
import com.sc.reminder.ui.calendar.antonyt.infiniteviewpager.InfiniteViewPager;
...
public class CaldroidFragment extends DialogFragment {
	...
	public void refreshView() {
		...
		LogUtils.e("t外面 = " + String.valueOf(t2 - t1));
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		monthTitleTextView.setShadowLayer(10F, 11F,5F, Color.BLACK);
		...
		for (int i = 0; i < NUMBER_OF_PAGES; i++) {
			...
			dateGridFragment.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> parent, View view,
					...
					DateTime dateTime = dateInMonthsList.get(position);
					HashMap<DateTime, Integer> disabled = adapter.getDisableDatesMap();
					if (disabled.containsKey(dateTime)) {
						return;
					}
					for(int i=0;i<parent.getCount();i++){
						RelativeLayout v = (RelativeLayout) parent.getChildAt(i);
						TextView rel_tv = (TextView) v.findViewById(R.id.cal_tv);
						ImageView cal_iv2 = (ImageView) v.findViewById(R.id.cal_iv2);
						DateTime dt = dateInMonthsList.get(i);
						if (position == i) {
							cal_iv2.setVisibility(View.VISIBLE);
							cal_iv2.setBackgroundResource(R.drawable.circle_inner_checked);
							if (!disabled.containsKey(dt) && adapter.getToday().equals(dt)) {
